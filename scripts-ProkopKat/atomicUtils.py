# use standard .bas files input for  QD and molecule handling
# place in the same folder as processed .xyz or .bas data together with elements.py, atomicUtils.py and plotUtils.py
        ...
        for line in f:
            ...
            if wds >= 4:
                try:
                    ...
                    except:  # this is for .xyz
                        ename = wds[0]
                        enames.append(ename)
                        Zs.append(elements.ELEMENT_DICT[ename][0])
                except:
                    print("cannot interpet line: %s" % (line))
                    continue
    xyzs = np.array(xyzs)
    Zs   = np.array(Zs,dtype=np.int32) #this dtype=np.int32 sets size of the integer for memory requirements etc... 
...
def findAllBonds( atoms, Rcut=3.0, RvdwCut=0.7 ): #in Angstroms, atoms=[[name1 x1 y1 z1],[name2 x2 y2 z2], ...]
    bonds     = []
    bondsVecs = []
    ps     = atoms[:,1:] # array of just x,y,z coordinates
    iatoms = np.arange( len(atoms), dtype=int ) #makes numpy array of length of number of atoms with integeres as the atom labels, i.e. [1 2 3 4 5 6 .... N] 
    Rcut2 = Rcut*Rcut
    for i,atom in enumerate(atoms):
        #enumerate adds number to each item (i.e. instead of [a b c d] we get [[1,a] [2,b] [3,c] [4,d])
        p    = atom[1:] #coordinates x y z 
        dp   = ps - p   #xi-xj, yi-yj, zi-zj 
        rs   = np.sum( dp**2, axis=1 ) #(xi-xj)^2+(yi-yj)^2+(zi-zj)^2
        for j in iatoms[:i][ rs[:i] < Rcut2 ]:#for those atoms that are closer than covalent bond cut length
            ei = int( atoms[i,0] )
            ej = int( atoms[j,0] )#save in ei and ej the label of the atom (i.e. "H" or "Si") 
            Rcut_ij = elements.ELEMENTS[ei][7] + elements.ELEMENTS[ej][7] #find how far would these atoms be if non-bonded (i.e. vdW of H plus vdW of Si)
            rij =  np.sqrt( rs[j] )#distance between the closest atoms
            if ( rij < ( RvdwCut * Rcut_ij ) ):#is the distance smaller than approximately 0.7 times the van der Waals added radii of the two elements??
                bonds.append( (i,j) )
                bondsVecs.append( ( rij, dp[j]/rij ) )# (rij,(xi-xj)/rij,(yi-yj)/rij,(zi-zj)/rij)
            print ( i, j, ei, ej, rij, Rcut_ij )
...
def neighs( natoms, bonds ):
    ...
    for ib, b in enumerate(bonds):
        ...
        neighs[i][j] = ib
        neighs[j][i] = ib
    return neighs #enumerated list of neighbours from the bonds (everything that is bonded to each other is also a neighbour)


def findTypeNeigh(atoms,neighs,typ,neighTyps=[(1,2,2)]):#typ is, if not specified otherwise, (1,2,2), i.e. hydrogen (1) attached in numbers min 2 and max 2 to the Si atom
