...
#python ../bas2bondlength_Kat.py 0.2
...
popt,pcov = curve_fit(Gauss, xx, yy, p0=[max(yy), mean, sigma])

meanr=round(mean,2)
sigmar=round(sigma,2)
result="mean="+ str(meanr) + " standard deviation=" + str(sigmar)
plt.plot(xx, yy, 'b+:')
plt.plot(xx, Gauss(xx, *popt), 'r-', label=result)
plt.legend()
plt.title('Si-Si bond')
