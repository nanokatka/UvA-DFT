...
def main():
    ...
    if save_bins:
        np.savetxt('kmap_binsGX.dat', bins,  header=" nE %i nk %i dE %f Emin %f Emax %f " %(bins.shape[0],bins.shape[1],dE,Emin,Emax) )    
    # ------- plot
    plt.figure(figsize=(3,8))
    plt.imshow( np.log10(bins), interpolation='nearest', origin='image', extent=extent, cmap='jet' )
    plt.colorbar()
    plt.savefig( "kvazibandGX.png", bbox='tight' )
    plt.show()
